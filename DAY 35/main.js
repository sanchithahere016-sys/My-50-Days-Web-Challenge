...
import {
    fetchGitHubUser,
    fetchGitHubRepos,
    submitInitiative as postInitiative,
    updateInitiative as putInitiative,
...
<section class="infinite-scroll-section">
    ...
    <h2>Infinite Scroll Data Feed</h2>
...
function initGitHubLookup() {
    ...
    async function handleLookup() {
        ...
        if (username === "") {
            ...
            if (reposGrid) reposGrid.innerHTML = "";
            return;
        }
    ...
    async function loadRepos(username) {
        ...
        try {
            ...
            reposGrid.innerHTML = `<p>Failed to load repositories.</p>`;
 
        }
 
    }
 
    const optimizedLookup = debounce(handleLookup, 500);
 
    usernameInput.addEventListener("input", optimizedLookup);
 
}
 
async function handleSubmitInitiative(proposalForm, submitBtn, feedbackMessage, dataPayload) {
    ...
    try {
        ...
        const serverResponse = await postInitiative(dataPayload);
...
function initDay29Form() {
    ...
    proposalForm.addEventListener('submit', (e) => {
        ...
        handleSubmitInitiative(proposalForm, submitBtn, feedbackMessage, newInitiative);
 
    });
 
}
 
function initDay30CRUD() {
    ...
    updateBtn.addEventListener("click", async () => {
        ...
        try {
            const data = await putInitiative(1);
    ...
    if (confirmDelete) {
        ...
        try {
            const data = await secureDeleteResource(1);

            console.log("Secure Delete Successful");
            console.log(data);
